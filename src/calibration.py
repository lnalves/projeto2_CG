...
O usuário clica em dois pontos de distância real
...
def calibrar_detalhado(imagem, config, regua_roi=None) -> ResultadoCalibracao:
    """Executa calibração manual por 2 cliques."""
    ...
    log.info(
        "Calibração [{}] confiança={:.2f} | escala={:.5f} mm/px ({:.2f} px/mm)",
        manual.metodo, manual.confianca, manual.mm_por_px, 1.0 / manual.mm_por_px,
    )
    return manual
...
def _calibrar_manual(imagem, cal) -> ResultadoCalibracao | None:
    ...
    if dist_px <= 0:
        log.error("Calibração: pontos coincidentes ({:.1f}px)", dist_px)
        return None
    if dist_px < 10:
        log.warning("Calibração: pontos muito próximos ({:.1f}px) — escala imprecisa.", dist_px)

    mm_por_px = cal.distancia_conhecida_mm / dist_px
    log.info(
        "Calibração manual: {}px → {:.1f}mm ⇒ {:.5f} mm/px",
        dist_px, cal.distancia_conhecida_mm, mm_por_px,
    )
    ...
    try:
        while True:
            ...
            if tecla == 27:
                cliques = []
                break
            if tecla in (13, 10) and len(cliques) == 2:
                break
    finally:
